const gallery = document.querySelector('.js-gallery') as HTMLUListElement;
const modal = document.querySelector('.js-lightbox') as HTMLDivElement;
...
const galleryItemMarkup = (image: Image): string => {
...
const findImage = (list: Element[]) => {
    return list.find((e) => e.classList.contains('lightbox__content'))
        ?.children[0] as HTMLImageElement;
};
const findIndexOfImage = () => {
    const src = findImage([...modal.children]).src;
    const currentImage = galleryItems.find((e) => e.original === src) as Image;
...
const openModel = (target: HTMLElement) => {
...
const onArrowRightClick = (e: KeyboardEvent) => {
...
const onArrowLeftClick = (e: KeyboardEvent) => {
    if (e.code === 'ArrowLeft') {
        const index = findIndexOfImage();
        const original = findImage([...modal.children]);
